...
static uint8_t vtype(jsval_t v) {
    return is_nan(v) ? ((v >> 48) & 15) : (uint8_t) T_NUM;
}
...
static jsval_t mkcoderef(jsval_t off, jsoff_t len) {
    return mkval(T_CODEREF, (off & 0xffffff) | ((len & 0xffffff) << 24));
}
...
static uint8_t unhex(uint8_t c) {
    return (c >= '0' && c <= '9') ? c - '0' : (c >= 'a' && c <= 'f') ? c - 'W' : (c >= 'A' && c <= 'F') ? c - '7' : 0;
}
...
static bool is_right_assoc(uint8_t tok) {
    return (tok >= TOK_NOT && tok <= TOK_UMINUS) || (tok >= TOK_Q && tok <= TOK_OR_ASSIGN);
}
...
static uint8_t parsekeyword(const char *buf, size_t len) {
    ...
    switch (buf[0]) {
        case 'b':
            if (streq("break", 5, buf, len)) return TOK_BREAK;
            break;
        case 'c':
        ...
        case 'e':
            if (streq("else", 4, buf, len)) return TOK_ELSE;
            break;
        case 'f':
        ...
        case 'i':
            ...
            break;
        case 'l':
            if (streq("let", 3, buf, len)) return TOK_LET;
            break;
        case 'n':
            ...
            break;
        case 'r':
            if (streq("return", 6, buf, len)) return TOK_RETURN;
            break;
        case 's':
            if (streq("switch", 6, buf, len)) return TOK_SWITCH;
            break;
        case 't':
            ...
            break;
        case 'u':
            if (streq("undefined", 9, buf, len)) return TOK_UNDEF;
            break;
        case 'v':
        ...
        case 'w':
            ...
            break;
        case 'y':
            if (streq("yield", 5, buf, len)) return TOK_YIELD;
            break;
    }
    // clang-format on
...
#define LOOK(OFS, CH) js->toff + OFS < js->clen && buf[OFS] == CH
    switch (buf[0]) {
        ...
        case '!':
            if (LOOK(1, '=') && LOOK(2, '=')) TOK(TOK_NE, 3);
            TOK(TOK_NOT, 1);
        case '.': TOK(TOK_DOT, 1);
        case '~': TOK(TOK_NEG, 1);
        case '-':
            if (LOOK(1, '-')) TOK(TOK_POSTDEC, 2);
            if (LOOK(1, '=')) TOK(TOK_MINUS_ASSIGN, 2);
            TOK(TOK_MINUS, 1);
        case '+':
            if (LOOK(1, '+')) TOK(TOK_POSTINC, 2);
            if (LOOK(1, '=')) TOK(TOK_PLUS_ASSIGN, 2);
            TOK(TOK_PLUS, 1);
        case '*':
            if (LOOK(1, '*')) TOK(TOK_EXP, 2);
            if (LOOK(1, '=')) TOK(TOK_MUL_ASSIGN, 2);
            TOK(TOK_MUL, 1);
        case '/':
            if (LOOK(1, '=')) TOK(TOK_DIV_ASSIGN, 2);
            TOK(TOK_DIV, 1);
        case '%':
            if (LOOK(1, '=')) TOK(TOK_REM_ASSIGN, 2);
            TOK(TOK_REM, 1);
        case '&':
            if (LOOK(1, '&')) TOK(TOK_LAND, 2);
            if (LOOK(1, '=')) TOK(TOK_AND_ASSIGN, 2);
            TOK(TOK_AND, 1);
        case '|':
            if (LOOK(1, '|')) TOK(TOK_LOR, 2);
            if (LOOK(1, '=')) TOK(TOK_OR_ASSIGN, 2);
            TOK(TOK_OR, 1);
        case '=':
            if (LOOK(1, '=') && LOOK(2, '=')) TOK(TOK_EQ, 3);
            TOK(TOK_ASSIGN, 1);
        case '<':
            if (LOOK(1, '<') && LOOK(2, '=')) TOK(TOK_SHL_ASSIGN, 3);
            if (LOOK(1, '<')) TOK(TOK_SHL, 2);
            if (LOOK(1, '=')) TOK(TOK_LE, 2);
            TOK(TOK_LT, 1);
        case '>':
            if (LOOK(1, '>') && LOOK(2, '=')) TOK(TOK_SHR_ASSIGN, 3);
            if (LOOK(1, '>')) TOK(TOK_SHR, 2);
            if (LOOK(1, '=')) TOK(TOK_GE, 2);
            TOK(TOK_GT, 1);
        case '^':
            if (LOOK(1, '=')) TOK(TOK_XOR_ASSIGN, 2);
            TOK(TOK_XOR, 1);
        case '"':
        case '\'':
            js->tlen++;
...
static jw_t fficb(uintptr_t param, union ffi_val *args) {
    ...
    switch (decl[1]) {
        ...
        case 'p':
            return (jw_t) tod(res);
        case 's':
            if (vtype(res) == T_STR) return (jw_t) (js->mem + vstr(js, res, NULL));
    }
    return res;
}
...
static jsval_t call_c(struct js *js, const char *fn, int fnlen, jsoff_t fnoff) {
...
#endif
            case 'd':
                if (t != T_NUM) return js_err(js, "bad arg %d", n + 1);
                args[n++].d = tod(v);
                break;
            case 'b':
                if (t != T_BOOL) return js_err(js, "bad arg %d", n + 1);
                args[n++].w = vdata(v);
                break;
            case 'i':
                if (t != T_NUM && t != T_BOOL) return js_err(js, "bad arg %d", n + 1);
                args[n++].w = t == T_BOOL ? (long) vdata(v) : (long) tod(v);
                break;
            case 's':
                if (t != T_STR) return js_err(js, "bad arg %d", n + 1);
    ...
    if (f == (uintptr_t) js_eval_buff) js->nogc = ~0;
#ifndef WIN32
#define __cdecl
#endif
    switch (type) {
        case 0:
            res.u64 = ((uint64_t(__cdecl *)(jw_t, jw_t, jw_t, jw_t, jw_t, jw_t)) f)(args[0].w, args[1].w, args[2].w,
                                                                                    args[3].w, args[4].w, args[5].w);
            break;
        case 1:
            res.d = ((double (__cdecl *)(jw_t, jw_t, jw_t, jw_t, jw_t, jw_t)) f)(args[0].w, args[1].w, args[2].w,
                                                                                 args[3].w, args[4].w, args[5].w);
            break;
        case 2:
            res.u64 = ((uint64_t(__cdecl *)(double, jw_t, jw_t, jw_t, jw_t, jw_t)) f)(args[0].d, args[1].w, args[2].w,
                                                                                      args[3].w, args[4].w, args[5].w);
            break;
        case 3:
            res.d = ((double (__cdecl *)(double, jw_t, jw_t, jw_t, jw_t, jw_t)) f)(args[0].d, args[1].w, args[2].w,
                                                                                   args[3].w, args[4].w, args[5].w);
            break;
        case 4:
            res.u64 = ((uint64_t(__cdecl *)(jw_t, double, jw_t, jw_t, jw_t, jw_t)) f)(args[0].w, args[1].d, args[2].w,
                                                                                      args[3].w, args[4].w, args[5].w);
            break;
        case 5:
            res.d = ((double (__cdecl *)(jw_t, double, jw_t, jw_t, jw_t, jw_t)) f)(args[0].w, args[1].d, args[2].w,
                                                                                   args[3].w, args[4].w, args[5].w);
            break;
        case 6:
            res.u64 = ((uint64_t(__cdecl *)(double, double, jw_t, jw_t, jw_t, jw_t)) f)(args[0].d, args[1].d, args[2].w,
                                                                                        args[3].w, args[4].w,
                                                                                        args[5].w);
            break;
        case 7:
            res.d = ((double (__cdecl *)(double, double, jw_t, jw_t, jw_t, jw_t)) f)(args[0].d, args[1].d, args[2].w,
                                                                                     args[3].w, args[4].w, args[5].w);
            break;
        default:
    ...
    switch (fn[0]) {
        ...
        case 'd':
            return tov(res.d);
        case 'b':
            return mkval(T_BOOL, res.w ? 1 : 0);
        case 's':
            return js_mkstr(js, (char *) (intptr_t) res.w, strlen((char *) (intptr_t) res.w));
        case 'v':
            return mkval(T_UNDEF, 0);
        case 'j':
...
static jsval_t call_js(struct js *js, const char *fn, int fnlen) {
    ...
    jsval_t res = js_eval_buff(js, &fn[fnpos], n);  // Call function, no GC
...
static jsval_t do_op(struct js *js, uint8_t op, jsval_t lhs, jsval_t rhs) {
    ...
    switch (op) {
        case TOK_LAND:
            return mkval(T_BOOL, js_truthy(js, l) && js_truthy(js, r) ? 1 : 0);
        case TOK_LOR:
        ...
        case TOK_NOT:
            if (vtype(r) == T_BOOL) return mkval(T_BOOL, !vdata(r));
            break;
    }
    ...
    switch (op) {
        ...
        case TOK_EQ:
            return mkval(T_BOOL, (long) a == (long) b);
        case TOK_NE:
            return mkval(T_BOOL, (long) a != (long) b);
        case TOK_LT:
...
static jsval_t js_stmt(struct js *js, uint8_t etok) {
    ...
    switch (nexttok(js)) {
        ...
        case TOK_BREAK:
            ...
            break;
            // let and var token
        case TOK_VAR: // todo var may not be replacable?
        case TOK_LET:
            res = js_let(js);
            break;
            // if token
...
jsval_t js_eval(struct js *js, const char *string) {
    return js_eval_buff(js, string, ~0);
}
